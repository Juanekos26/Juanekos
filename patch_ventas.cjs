...
const filterLogicOld = `    if (estadoSeleccionado) {
        pedidos =
            pedidos.filter(
                pedido =>
                    normalizarEstado(
...
const filterLogicNew = `    if (estadoSeleccionado) {
    ...
    if (window.categoriaPedidoActiva && window.categoriaPedidoActiva !== 'todos') {
        pedidos = pedidos.filter(pedido => {
            ...
            if (window.categoriaPedidoActiva === 'cevicheria') return esCev && !esBros;
...
const eventsOld = `function configurarFiltrosVentas() {
    const fecha =
...
console.log('Patched panel-ventas.js');
